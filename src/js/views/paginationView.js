...
class PaginationView extends View {
  ...
  _generateMarkup() {
    ...
    if (curPage === 1 && numPages > 1) {
      return this._generateMarkupNextPage(curPage + 1);
    }

    if (curPage === numPages && numPages > 1) {
      return this._generateMarkupPrevPage(curPage - 1);
    }

    if (curPage < numPages) {
      return [
        this._generateMarkupPrevPage(curPage - 1),
        this._generateMarkupNextPage(curPage + 1),
      ].join('');
    }
    return '';
  }
